...
class BaseRequestor(object, metaclass=abc.ABCMeta):
  ...
  def Request(self, service_name, message_name, request_datum={}):
    ...
    self._WriteHandshakeRequest(buffer_encoder, service_name, message_name, timeout, protocol_type)
  ...
  def _WriteHandshakeRequest(self, encoder, service_name, message_name, timeout, protocol_type):
...
class SocketTransceiver(Transceiver):
  """Socket-based transceiver implementation."""

  def __init__(self, host, port, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
    """Initializes a new Socket transceiver.
    ...
    # self._conn = socket.socket(socket.AF_INET,socket.SOCK_STREAM)      #定义socket类型，网络通信，TCP
    # self._conn.connect((host, port))
    self._remote_name = property(lambda self: self._conn.getsockname())
  ...
  def Transceive(self, request):
      self._conn = socket.create_connection((self._host, self._port), self._timeout)
